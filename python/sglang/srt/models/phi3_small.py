...
from sglang.srt.layers.logits_processor import LogitsProcessor, LogitProcessorOutput
...
class Phi3SmallDecoderLayer(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = Phi3SmallSelfAttention(config=config,
                                                layer_id=layer_id,
                                                quant_config=quant_config)
...
class Phi3SmallModel(nn.Module):
    ...
    def __init__(
        ...
        self.layers = nn.ModuleList([
            Phi3SmallDecoderLayer(config, layer_id, quant_config) for layer_id in range(config.num_hidden_layers)
        ])
...
class Phi3SmallForCausalLM(nn.Module):
    ...
    def __init__(
        ...
        self.model = Phi3SmallModel(config, quant_config=quant_config)
    ...
    def forward(
        ...
        logits_output = self.logits_processor(
            input_ids, output_hidden_states, self.lm_head.weight, input_metadata)
